...
/*   Updated: 2025/08/14 09:57:15 by thodavid         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	fork_mutex_arr_init(t_data *data)
{
	...
	if (data->forks_arr == NULL)
		return (ft_error("φ forks_arr malloc failed φ"), (0));
	while (i < nb_ofork)
	{
		data->forks_arr[i].id = i;
		if (pthread_mutex_init(&data->forks_arr[i].mutex, NULL) != 0)
		{
			ft_error("φ mutex_init err φ(3)");
			while (j < i)
				pthread_mutex_destroy(&data->forks_arr[j++].mutex);
			return (free(data->forks_arr), data->forks_arr = NULL, (0));
		}
		i++;
	}
	return (1);
